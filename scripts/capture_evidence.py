...
WHAT IT WRITES.
    ...
    docs/STATE_MACHINE.md     the transition table, rendered from the machine itself

The state machine document said it was generated long before anything generated it, and nothing
compared it to the code either. It was a hand-maintained copy of a table, carrying a bold
instruction not to edit it, which is the worst of both: a reader trusts it and a machine that
drifts from it says nothing.
...
#: Everything in the document that is not the table. The table is appended by `render_table()`,
#: so the only prose a reader can change is here, where changing it is obviously a code edit.
STATE_MACHINE_PREAMBLE = """# The state machine, generated from the code
...
def main() -> int:
    ...
    # Imported here rather than at the top, following `release()`: this script is run from the
    # repository root and the package is what it is reporting on, not a dependency of reporting.
    from quarantinez.states import render_table

    STATE_MACHINE.write_text(STATE_MACHINE_PREAMBLE + render_table() + "\n", encoding="utf-8")
